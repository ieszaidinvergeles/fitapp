...
<header class="fixed top-0 left-0 w-full px-6 h-16 flex justify-between items-center z-50">
    <div class="text-[#d7ff00] text-2xl font-black tracking-[-0.04em] font-headline uppercase">
        VOLT GYM
    </div>
</header>
...
<main class="relative z-20 min-h-screen flex flex-col items-center justify-center px-4 py-20">
    <div class="w-full max-w-md">
        <div class="mb-12 text-center md:text-left">
            <span class="text-primary-container font-headline font-bold uppercase tracking-[0.2em] text-xs mb-2 block">
                Performance Portal
            </span>
        ...
        <div class="bg-surface-container-high/80 backdrop-blur-xl rounded-[2.5rem] p-8 md:p-10 shadow-2xl border border-outline-variant/10">
            ...
            <div class="relative flex bg-surface-container-lowest rounded-full p-1.5 mb-10 overflow-hidden" id="login-toggle">
                ...
                <button type="button" class="relative z-10 flex-1 py-3 text-sm font-headline font-bold uppercase tracking-wider text-on-primary-container transition-colors duration-300" id="user-tab" onclick="switchTab('user')">
                    User
                </button>

                <button type="button" class="relative z-10 flex-1 py-3 text-sm font-headline font-bold uppercase tracking-wider text-on-surface-variant transition-colors duration-300" id="employee-tab" onclick="switchTab('staff')">
                    Staff
                </button>
            </div>
            ...
            <form method="POST" action="<?= esc_url(home_url('/')) ?>">
                ...
                <div id="user-fields" class="space-y-6">
                    ...
                    <div class="space-y-2">
                        <label class="text-[10px] font-label font-black uppercase tracking-widest text-on-surface-variant px-1">
                            Email Address
                        </label>

                        <input
                            class="w-full bg-surface-container-highest border-none rounded-xl px-4 py-4 focus:ring-2 focus:ring-primary-container text-on-surface placeholder:text-outline font-medium transition-all"
                            name="email"
                            placeholder="runner@apex.com"
                            type="email"
                            value="<?= h($_POST['email'] ?? '') ?>"
                        >
                    </div>
                    ...
                    <div class="space-y-2">
                        <div class="flex justify-between items-center px-1">
                            <label class="text-[10px] font-label font-black uppercase tracking-widest text-on-surface-variant">
                                Password
                            </label>

                            <a class="text-[10px] font-label font-black uppercase tracking-widest text-primary-fixed-dim hover:text-primary transition-colors" href="<?= esc_url(home_url('/?pagename=forgot-password')) ?>">
                                Forgot?
                            </a>
                        </div>
                ...
                <div id="staff-fields" class="space-y-6 hidden">
                    ...
                    <div class="space-y-2">
                        ...
                        <input
                            class="w-full bg-surface-container-highest border-none rounded-xl px-4 py-4 focus:ring-2 focus:ring-primary-container text-on-surface placeholder:text-outline font-medium transition-all"
                            name="staff_email"
                            placeholder="staff@voltgym.com"
                            type="email"
                            value="<?= h($_POST['staff_email'] ?? '') ?>"
                        >
                    </div>
            ...
            <p class="mt-10 text-center text-[10px] font-label font-bold uppercase tracking-widest text-on-surface-variant">
                Not a member?
                <a class="text-primary-container hover:underline underline-offset-4 decoration-2" href="<?= esc_url(home_url('/?pagename=register')) ?>">
                    Join the Elite
                </a>
            </p>
        </div>
...
<div class="fixed bottom-10 left-10 hidden md:block z-30">
    <div class="flex flex-col gap-4">
        <div class="w-1 h-20 bg-primary-container/20 relative rounded-full overflow-hidden">
            <div class="absolute top-0 left-0 w-full h-1/2 bg-primary-container rounded-full"></div>
        </div>

        <span class="text-[10px] font-headline font-bold uppercase tracking-[0.5em] [writing-mode:vertical-lr] text-primary-container">
            Stability_v2.4
        </span>
    </div>
</div>
