...
$response = api_get('/me/notifications?page=' . $page, auth: true);

// DEBUG BLOCK - REMOVE AFTER FIXING
if (isset($_GET['debug_api'])) {
    echo '<pre style="background:#000; color:#0f0; padding:20px; border:2px solid #0f0; margin:20px; overflow:auto; max-height:400px; z-index:9999; position:relative;">';
    echo "API RESPONSE:\n";
    print_r($response);
    echo '</pre>';
}
