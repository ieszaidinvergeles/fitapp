<?php if (empty($GLOBALS['hide_global_footer'])): ?>
<footer class="border-t border-outline-variant/20 mt-16">
    <div class="max-w-7xl mx-auto px-4 md:px-6 py-8 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
            <p class="font-headline font-extrabold uppercase tracking-wide text-primary-container">Volt Gym</p>
            <p class="text-xs uppercase tracking-widest text-on-surface-variant">Despierta el trueno</p>
        </div>
        <p class="text-xs text-on-surface-variant"></p>
    </div>
</footer>
<?php endif; ?>
</body>
</html>
