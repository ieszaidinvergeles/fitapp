...
function equipment_public_image_url(string $url): string
{
    if ($url === '' || $url === '-' || $url === '—') {
        return '';
    }

    return str_replace('http://nginx:8000', 'http://localhost:8000', $url);
}

/**
 * Enviar formulario multipart a Laravel.
 * Usamos _method=PUT para que Laravel lo trate como update.
 */
function equipment_api_multipart_update(string $endpoint, array $fields, ?array $file = null, bool $auth = true): array
{
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_equipment_submit'])) {
    ...
    $update_response = equipment_api_multipart_update(
...
<div class="space-y-6 pb-28">
    ...
    <?php if ($equipment): ?>
        <section class="rounded-3xl border border-outline-variant/20 bg-surface-container p-4 sm:p-6 shadow-lg">
            <form method="post" action="<?= esc_url($edit_equipment_url) ?>" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-[1fr_180px]">
                        ...
                        >
                            ...
                            <span class="text-sm font-bold text-on-surface">Change equipment image</span>
